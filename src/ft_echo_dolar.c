...
static int var_strcmp_echo(char *s1, char *s2) //, t_msh *d) //funcion strcmp modificada para variables de entorno
{
	int i;
...
char *ft_split_var(char *line, int i, t_msh *d) //funcion que retorna el resto de una variable si hay caracteres especiales. Ej: echo "$USER@hola" esta funcion retornara @hola
{
	int j;
	char *res;
...
int search_char(char *s, char c) //funcion que devuelve true o false si encuentra un char especifico en una string 1 = False | 0 = true
{
	int i;
...
char *ft_change_var(t_msh *d, char *line) //funcion para detectar y cambiar el valor a la linea y que si hay un caracter especial despues de $var se concatene. Ej: $USER/aaa 
{
	char *name_var;
	int i;
	char *result;
...
char *change_dolar_x_var(t_msh *d)
{
	...
	//if (d->read_line == NULL) //creo que este if ya sobra 
	//	d->read_line = ft_strdup("echo");
