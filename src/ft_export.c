...
void del_exist_variable(char *full_var, t_msh *d)
{
	int i;
	char *var;
	int len;
...
void add_new_vars2(t_msh *d, char *binary_array, int i, char **aux)
{
	int j;
...
int add_new_vars1(t_msh *d, char *binary_array)
{
	char **aux;
	int i;
	int new_envs;
...
char *check_same_var(t_msh *d, char *aux) //funcion para checkear si hay una variable de entorno igual que lo que me han mandado por parametro
{
	int i;
	char *result;
...
char *replace_dolar(t_msh *d, char *var, int i) //funcion para añadir una string que sea lo que va despues del $
{
	char *aux;
	int j;
...
char *value_var(t_msh *d, char *var) //funcion main para crear una variable nueva que contenga un $variable. Ej a=abc b=$a  el valor de $b es abc.
{
	char *result;
	int i;
	int j;
